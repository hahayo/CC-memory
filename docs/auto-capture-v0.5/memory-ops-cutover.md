...
只有「帶 `project_id`、`timestamp` 可解析且 < cutoff」的 event 記錄行才算舊時代證據：Stop sentinel（`hooks/stop-capture-sentinel.sh` 只寫 `transcript_path`／`hwm_offset`）與 timestamp 缺失或格式異常的記錄行都不算，snapshot 裡沒有任何舊時代證據時（例如 event 已在前一個 tick 消耗、只剩 sentinel）一律維持既有 id、不開 transcript（Codex 2026-09-03 R1 high：否則新 hook 寫的正確 id 會被 transcript 第一個 cwd 蓋掉）。sentinel-only 時記錄行沒有 `project_id`，fallback 改用 spool 目錄名經 `decodeSpoolSegment`（`sanitizeSpoolSegment` 反函數）還原的原始 id，不再把 `_uXXXX` 編碼名送進 LLM／DB。transcript 讀取只接受 regular file（`lstat`，不跟隨 symlink）並多讀 1 byte 判定截斷。
