...
Environment=CC_CAPTURE_QUIET_PERIOD_MS=7200000
...
**與 MCP server 一致（2026-09-02 使用者拍板選 A）**：`resolveProjectId` 第 4 層由 git-origin `owner/repo` 改為 git 根目錄名（`src/services/projects.ts`），`src/utils/repo-name.ts` 移除；server 與 hook 同為「explicit → env → marker → git 根目錄名 → cwd 目錄名」（前兩層只有 server 有）。DB 內從未出現 `owner/repo` 形式 id，不需資料遷移；跨裝置一致性改靠 CLAUDE.md marker 或相同 clone 目錄名。決策卡見 `docs/decisions/DEC-20260902T151857Z-align-project-id-with-capture-hooks.md`。

`project_id` 以原始字串（含中文）寫進 spool 記錄行；spool 目錄名／session 檔名用 `_uXXXX` 編碼（bash `sanitize_segment` 與 TypeScript `sanitizeSpoolSegment` 同構；**不是可逆編碼**，見下）：`[A-Za-z0-9.-]` 原樣、`_` 後面接 `u` 時編成 `_u005f`、第一個字元若是 `.` 編成 `_u002e`、其餘每個字元編成 `_uXXXX`（碼位十六進位）、空字串 → `unknown`。編碼不定長且無分隔，所以**不是單射**：`中a`→`_u4e2da`（與 `U+4E2DA` 同碼）、`U+1000`+`00` 與 `U+100000` 同為 `_u100000`——BMP 字元後接 `[0-9a-f]` 的 id 可能與另一個 id 共用目錄（project isolation 風險，Codex 2026-09-03 指出；另一個刻意重疊是空 id 與字面 `unknown`）。舊版一律換 `_` 讓所有中文名崩成同一目錄，比現在嚴重得多。要根治須改成固定寬度或帶終止符的 versioned encoding（另案，需同時改 hook）。worker 對新 hook 寫的記錄行以 `project_id` 為準（見下段 backlog 規則）。

**舊 backlog 的 project_id 重新歸屬（2026-09-03 起，worker 端）**：記錄行 `timestamp` 早於 `2026-09-02T22:10:00+08:00`（舊 hook 時代，id 已崩塌）的 session，worker 改從 transcript 開頭 256 KiB 內第一個 `cwd`（Claude transcript 每筆 entry 的頂層 `cwd`；Codex rollout 的 `session_meta.payload.cwd`）經 `resolveProjectId({ cwd, cwdIsExplicit: true })` 推導 project_id，並在 stdout 印 `project-id-remapped session=… old=… new=…`；同一 snapshot 內只要有任一記錄行 `timestamp >= cutoff`，一律以該記錄行 `project_id` 為準、不開 transcript。transcript 沒有 `cwd`、讀不到或解析失敗 → 維持記錄行 `project_id`；推導出 `__personal__` 走既有 skip 路徑；`cwd` 目錄已不存在時往上找不到 `.git` 就退到 `basename(cwd)`。rollup key `capture:v05:<project>:<session>` 用推導後的 id，因此同一 session 若 DB 已有舊 id 的 rollup 會多出一列新 id 的 rollup，由 DB 端重歸屬（對照表 `remap-2026-09-03.jsonl`）合併。**已知限制**：舊時代 session 中途 `cd` 到別的 repo 仍會整段歸到第一個 `cwd` 的專案——接受，比 `__` 好。

只有「帶 `project_id`、`timestamp` 可解析且 < cutoff」的 event 記錄行才算舊時代證據：Stop sentinel（`hooks/stop-capture-sentinel.sh` 只寫 `transcript_path`／`hwm_offset`）與 timestamp 缺失或格式異常的記錄行都不算，snapshot 裡沒有任何舊時代證據時（例如 event 已在前一個 tick 消耗、只剩 sentinel）一律維持既有 id、不開 transcript（Codex 2026-09-03 R1 high：否則新 hook 寫的正確 id 會被 transcript 第一個 cwd 蓋掉）。時代證據看整個 spool 檔（cursor 之前已消耗的記錄行也算），所以升級前已存在、只剩 sentinel 待處理的舊檔也能推導（Codex R1c）。每個 spool 檔推導出的 id 寫進 state 檔 `projectId`，之後的 snapshot（含 sentinel-only）沿用、不再重讀 transcript；新 hook 的 event 是權威來源，可覆寫 state（穩定路由下值相同）；目錄名解碼與 transcript 缺失時的記錄行 fallback 都**不**寫 state，之後有更好證據仍可修正。sentinel-only 且 state 還沒有 `projectId` 時（spool 檔第一個 snapshot 就是 sentinel），fallback 用 spool 目錄名經 `decodeSpoolSegment` 做 best-effort 還原——注意 `_uXXXX` 編碼不定長、無分隔，本質上不可逆（`中a`→`_u4e2da`、`😀`→`_u1f600`、`U+1000`+`00` 與 `U+100000` 同碼），解碼固定取 4 位十六進位（BMP 優先），BMP 外字元會解錯；這條路徑只剩極少數檔會走到。Codex R1b 另指出「舊時代 event＋新 hook 的 sentinel 同在一個 snapshot」仍會走推導：這其實是每個舊 backlog 檔的正常形狀（event＋sentinel），而新 hook 的 sentinel 只會寫進新 id 的 spool 目錄，只有舊 id 與新 id 剛好相同（ASCII repo 根目錄、無 marker）時才會落在同一檔，此時推導結果也是同一 id，唯一例外就是上面已接受的中途 `cd` 限制——Codex R1c 再給一個非 `cd` 的反例：舊 id == 新 id 的檔案，Stop 之後、worker 處理之前 CLAUDE.md marker 被改掉（TOCTOU），推導會讀到新 marker。這需要「session 跨 cutoff 且 cutoff 後沒有任何工具呼叫」＋「事後改 marker」同時成立，判為可接受的殘餘風險；故不視為阻擋項，留給使用者裁決是否要讓 sentinel 補 metadata（personal task #c7b7974a）。transcript 讀取只接受 regular file（`lstat`，不跟隨 symlink）並多讀 1 byte 判定截斷。
