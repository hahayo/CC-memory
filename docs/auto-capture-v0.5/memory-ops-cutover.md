...
只有「帶 `project_id`、`timestamp` 可解析且 < cutoff」的 event 記錄行才算舊時代證據：Stop sentinel（`hooks/stop-capture-sentinel.sh` 只寫 `transcript_path`／`hwm_offset`）與 timestamp 缺失或格式異常的記錄行都不算，snapshot 裡沒有任何舊時代證據時（例如 event 已在前一個 tick 消耗、只剩 sentinel）一律維持既有 id、不開 transcript（Codex 2026-09-03 R1 high：否則新 hook 寫的正確 id 會被 transcript 第一個 cwd 蓋掉）。每個 spool 檔第一次決定 project_id 後寫進 state 檔 `projectId`（Codex R1b），之後的 snapshot（含 sentinel-only）直接沿用、不再重讀 transcript，跨 tick 一致。sentinel-only 且 state 還沒有 `projectId` 時（spool 檔第一個 snapshot 就是 sentinel），fallback 用 spool 目錄名經 `decodeSpoolSegment` 做 best-effort 還原——注意 `_uXXXX` 編碼不定長、無分隔，本質上不可逆（`中a`→`_u4e2da`、`😀`→`_u1f600`、`U+1000`+`00` 與 `U+100000` 同碼），解碼固定取 4 位十六進位（BMP 優先），BMP 外字元會解錯；這條路徑只剩極少數檔會走到。Codex R1b 另指出「舊時代 event＋新 hook 的 sentinel 同在一個 snapshot」仍會走推導：這其實是每個舊 backlog 檔的正常形狀（event＋sentinel），而新 hook 的 sentinel 只會寫進新 id 的 spool 目錄，只有舊 id 與新 id 剛好相同（ASCII repo 根目錄、無 marker）時才會落在同一檔，此時推導結果也是同一 id，唯一例外就是上面已接受的中途 `cd` 限制——故不視為阻擋項，留給使用者裁決是否要讓 sentinel 補 metadata（personal task #c7b7974a）。transcript 讀取只接受 regular file（`lstat`，不跟隨 symlink）並多讀 1 byte 判定截斷。
