...
> 目標：worker 從 spool harvest（收割）batch（批次），一次呼叫 capture LLM（程式碼預設 claude-cli；2026-07-07 拍板前為 Gemini Flash；2026-08-23 正式 unit 改 codex-cli 主力 + claude-cli fallback），驗證 JSON schema（結構），寫 rollup + observations。
...
- [ ] `CC_CAPTURE_LLM` 程式碼預設 claude-cli（2026-07-07 拍板；原 Gemini Flash 交付後變更，gemini-flash 保留可切）；正式 unit 以 `CC_CAPTURE_LLM=codex-cli` 覆蓋為主力、`CC_CAPTURE_LLM_FALLBACK=claude-cli` 自動退回；`CC_CAPTURE_CLAUDE_MODEL` 預設 haiku；`CC_CAPTURE_CODEX_MODEL` 預設 gpt-5.6-sol
- [ ] codex-cli 主力失敗時自動 fallback 到 `CC_CAPTURE_LLM_FALLBACK` 所指 provider
- [ ] provider 不可用（claude CLI 不存在／codex CLI 不存在／gemini 缺 `GEMINI_API_KEY`）時靜默停用 capture，stdout 告警
...
- [ ] Coolify 單一 PostgreSQL stack 的雙 DB 備份已改為固定 `backup-project`／`backup-personal` 兩個 idle services，compose contract tests 4/4 PASS；`.dockerignore` 已把 backup image build context 限制為 Dockerfile 與備份腳本。尚未 push／部署，也尚未建立兩筆錯開的每日 Scheduled Tasks。完成後須觀察至少一個自動週期，確認兩側新 manifest 與 freshness 均 PASS
