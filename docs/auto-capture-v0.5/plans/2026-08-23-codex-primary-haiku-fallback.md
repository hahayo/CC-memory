...
- 上傳沿用既有 `scripts/upload-capture-backlog.ts` 的加密 R2 管線（該腳本本身不動 spool）。
  - **2026-08-27 後記（實跑修正）**：本節原文寫「沿用既有上傳管線」，但該路徑從未實跑，測試也未覆蓋。首次執行時失敗於 `capture archive verification failed: unexpected archive entry outside the manifest allowlist`——上傳 CLI 只呼叫 cutoff 專用的 `verifyCaptureEpochArchive`，而 copy-live archive 的 manifest 欄位（`mode`／`snapshotId`／`snapshotAt`）與 spoolFiles 形狀都不同，且當時 copy-live 會把未壓縮的 `spool/` 暫存目錄留在 archive 內，違反允許清單。已修正三處：(1) `verifyCopyLiveArchive` 補上 tar 解壓逐檔比對與允許清單檢查——檔案面的檢查項目與 cutoff 驗證器對齊，目錄面則**比 cutoff 更嚴**：cutoff 驗證器只要求 tar 內目錄都在 spool root 之下，copy-live 驗證器另要求每個目錄都是某個 manifest 檔案的祖先，未列出的空目錄一律拒絕（Codex R2 以實跑重現該空目錄可矇混後補上）；(2) `executeCopyLiveSnapshot` 打包後刪除未壓縮暫存目錄；(3) 上傳 CLI 依 manifest 的 `mode` 欄位分派驗證器，並在提交的 R2 manifest 新增 `source_mode`。R2 manifest 的 `source_cutoff_id`／`source_cutoff_at` 欄位名保留，copy-live 時承載 snapshot 值，故 freshness checker 等既有讀取端不受影響。操作步驟見 `memory-ops-cutover.md` §5.0。Codex 對審另抓到一項（P1，已採納）：新驗證器會把 `spool.tar.gz` 整包解開比對，但上傳 CLI 原本在**驗證之後**才做 tmpfs 容量檢查，且公式只算 archive 大小×3——高壓縮率的 JSONL 因此可能通過文件標示的容量指引卻在驗證中途 `ENOSPC`。已改為容量檢查前置，並把 manifest 宣告的未壓縮 spool 大小納入公式；`executeCopyLive` 的驗證解壓也改到 archive 所在檔案系統，不再落在預設 `/tmp`。
