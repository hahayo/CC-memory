...
- **Session start latency** — the injector uses a 2s DB connect timeout, plus the hook wrapper applies `timeout -k 1 5` (SIGKILL 1s after SIGTERM) as a hard overall cap for `npx` / loader stalls; every failure path fails open (empty stdout). If session start feels slow, unset `CC_MEMORY_INJECT_RECENT` to disable.
