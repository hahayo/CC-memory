...
- `CC_DB_CONNECT_TIMEOUT_SEC` - auto-capture worker 連 PostgreSQL 的 connect timeout（連線逾時）秒數（預設 2）。高並行 drain 共用同一條 SSH tunnel（SSH 通道）時 2 秒太短（2026-09-20 實測 10 支 6 小時內 32 次 CONNECT_TIMEOUT），補舊帳 runner 設 15；必須是完整的正整數字串，否則一律退回預設 2（打錯字如 `15seconds`、`2.5` 都算，不會被截成 15／2）
