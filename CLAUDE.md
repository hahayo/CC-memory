...
- `CC_MEMORY_WEIGHT_MANUAL` - search ranking（搜尋排序）source weight（來源權重）：manual 記憶（預設 `1.00`）；parse 失敗用預設
...
- `CC_MEMORY_WEIGHT_SESSION_RECENCY_MIN` - 同 session 內 observation 的時序加權（session recency weighting）最低乘數（預設 `0.90`）：最早 observation 得此乘數、最晚得 `1.0`、中間線性內插（dense-rank）；設 `1.0` 關閉；parse 失敗用預設
- `CC_MEMORY_WEIGHT_ROLLUP_SESSION_FLOOR` - rollup 的加權分數下限保護（rollup session floor）：同 session 的 rollup 不低於該 session 內最高 observation 加權分數（預設 `1`＝啟用）；設 `0` 關閉；parse 失敗用預設
