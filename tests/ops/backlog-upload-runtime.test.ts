...
function installFakeBins(binDir: string): void {
...
describe('encrypted R2 backlog archive upload CLI', () => {
  ...
  it('removes orphaned copy-live verifier directories after acquiring the upload lock', () => {
    // An upload killed mid-verification leaves the whole uncompressed spool
    // here; if the sweep does not recognise the prefix it blocks later uploads.
    const orphan = join(workDir, 'cc-memory-copy-live-verify-stale-run');
    mkdirSync(orphan);
    writeFileSync(join(orphan, 'plaintext.jsonl'), 'sensitive', { mode: 0o600 });
...
// ---------------------------------------------------------------------------
// copy-live archives: the read-only snapshot shape produced by
// `archive-capture-backlog --copy-live --execute`. It carries `mode:
// 'copy-live'` and `snapshotId`/`snapshotAt` instead of `cutoffId`/`cutoffAt`,
// so the uploader must route it to verifyCopyLiveArchive.
// ---------------------------------------------------------------------------

describe('encrypted R2 upload CLI with a copy-live archive', () => {
  let root: string;
  let binDir: string;
  let workDir: string;
  let remoteDir: string;
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'cc-memory-copy-live-upload-'));
    binDir = join(root, 'bin');
    workDir = join(root, 'work');
    remoteDir = join(root, 'remote');
    commandLog = join(root, 'commands.log');
    const spoolDir = join(root, 'spool');
    archiveDir = join(root, 'copy-live-20260825T034111Z');
    mkdirSync(binDir);
    mkdirSync(workDir);
    mkdirSync(remoteDir);
    mkdirSync(join(spoolDir, 'project'), { recursive: true });
    writeFileSync(join(spoolDir, 'project', 'session.jsonl'), '{"a":1}\n', 'utf8');
    writeFileSync(
      join(spoolDir, 'project', 'session.capture-state.json'),
      '{"cursor":8}',
      'utf8',
    );
    executeCopyLiveSnapshot({
      ...
      snapshotId: '20260825T034111Z',
      snapshotAt: '2026-08-25T03:41:11.718Z',
    });
    installFakeBins(binDir);
  });
  ...
  function run(overrides: NodeJS.ProcessEnv = {}) {
    ...
    return spawnSync(
      process.execPath,
      [TSX_CLI, UPLOAD_SCRIPT, '--archive-dir', archiveDir, '--json'],
      { env: childEnv, encoding: 'utf8' },
    );
  }

  it('verifies, encrypts, reads back and commits a copy-live archive', () => {
    const result = run();

    expect(result.status, result.stderr).toBe(0);
    const output = JSON.parse(result.stdout) as { ok: boolean; objectKey: string };
    expect(output).toMatchObject({
      ok: true,
      objectKey: expect.stringMatching(
        /^backups\/v1\/backlog\/2026\/08\/20260825T034111Z-abcdef0123456789\.tar\.gz\.age$/,
      ),
    });
    ...
    // The committed manifest records copy-live provenance; the legacy key names
    // are kept so existing readers keep parsing, carrying the snapshot id/time.
    expect(manifest).toMatchObject({
      schema_version: 1,
      target: 'backlog',
      source_mode: 'copy-live',
      source_cutoff_id: '20260825T034111Z',
      source_cutoff_at: '2026-08-25T03:41:11.718Z',
    });
    expect(manifest.archive_counts.spoolFiles).toBe(2);
    // The local archive must stay pristine.
  ...
  it('refuses a copy-live archive whose spool.tar.gz no longer matches the manifest', () => {
    ...
    writeFileSync(
      join(archiveDir, 'manifest.sha256'),
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('capture archive verification failed');
    expect(readdirSync(remoteDir)).toEqual([]);
  });

  it('refuses a manifest declaring an unsupported mode', () => {
    const manifestPath = join(archiveDir, 'manifest.json');
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
    manifest.mode = 'something-else';
    const source = `${JSON.stringify(manifest, null, 2)}\n`;
    writeFileSync(manifestPath, source, { mode: 0o600 });
    writeFileSync(
      join(archiveDir, 'manifest.sha256'),
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('unsupported mode');
    expect(readdirSync(remoteDir)).toEqual([]);
  });
});

// ---------------------------------------------------------------------------
// Temporary-space accounting: both verifiers untar spool.tar.gz into the tmp
// dir, so the capacity check must run BEFORE any verification and must budget
// for the uncompressed spool the manifest declares.
// ---------------------------------------------------------------------------

describe('tmpfs capacity accounting for extraction', () => {
  let root: string;
  let binDir: string;
  let workDir: string;
  let remoteDir: string;
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'cc-memory-capacity-'));
    binDir = join(root, 'bin');
    workDir = join(root, 'work');
    remoteDir = join(root, 'remote');
    commandLog = join(root, 'commands.log');
    ...
    mkdirSync(remoteDir);
    mkdirSync(join(spoolDir, 'project'), { recursive: true });
    // Highly compressible content: a small spool.tar.gz that expands a lot.
    const line = `${JSON.stringify({ pad: 'a'.repeat(4000) })}\n`;
    writeFileSync(join(spoolDir, 'project', 'session.jsonl'), line.repeat(200), 'utf8');
    executeCopyLiveSnapshot({
      spoolDir,
      stagingDir: archiveDir,
      snapshotId: '20260825T034111Z',
  ...
  function run(availableKb: string) {
    writeExecutable(join(binDir, 'fake-df'), `#!/usr/bin/env bash
set -euo pipefail
printf 'df %s\\n' "$*" >> "$COMMAND_LOG"
printf '%s\\n' 'Filesystem 1024-blocks Used Available Capacity Mounted on'
printf '%s\\n' 'tmpfs 4194304 0 ${availableKb} 0% /work'
`);
    const childEnv: NodeJS.ProcessEnv = {
      ...process.env,
      PATH: `${binDir}:${process.env.PATH ?? ''}`,
  ...
  it('refuses when free space covers the archive but not the extracted spool', () => {
    // spool.tar.gz compresses ~800 KB of padding to a few KB. Offer enough for
    // archive × 3 + 64 MiB but not for the declared uncompressed spool on top.
    const archiveBytes = readdirSync(archiveDir, { recursive: true, withFileTypes: false })
      .map(String)
      .map((name) => join(archiveDir, name))
      .filter((path) => statSync(path).isFile())
      .reduce((sum, path) => sum + statSync(path).size, 0);
    const declared = JSON.parse(readFileSync(join(archiveDir, 'manifest.json'), 'utf8'))
      .spoolFiles.reduce((sum: number, file: { approvedBytes: number }) => sum + file.approvedBytes, 0);
    // Sanity: the spool really does expand well beyond the archive on disk.
    expect(declared).toBeGreaterThan(archiveBytes);

    const withoutSpool = archiveBytes * 3 + 64 * 1024 * 1024;
    const result = run(String(Math.floor((withoutSpool + declared / 2) / 1024)));

    expect(result.status).not.toBe(0);
    expect(result.stderr).toMatch(/insufficient tmpfs capacity/);
    // The capacity guard must fire before any extraction happens.
    expect(readdirSync(remoteDir)).toEqual([]);
  });

  it('proceeds when free space also covers the extracted spool', () => {
    const result = run('4194304');
    expect(result.status, result.stderr).toBe(0);
  });
});
